...
func (g *GitRepo) Initialise(workingDir string) error {
...
func (g *GitRepo) GetHeadCommitID() string {
	...
	repo, err := git.PlainOpen(g.WorkingDir)
	if err != nil {
		return headCommitID
	}
	ref, err := repo.Head()
...
func (g *GitRepo) Clone() error {
	...
	_, err = git.PlainClone(g.WorkingDir, false, &git.CloneOptions{
		URL:           g.CloneID,
		Auth:          publicKey,
...
func (g *GitRepo) Pull() error {
	...
	if err != nil {
		...
		case "already up-to-date":
			// Log.Debugf("Already up-to-date for '%s'", g.WorkingDir)
			err = nil
		case "non-fast-forward update":
			// Log.Debugf("Non-fast-forward update for '%s'", g.WorkingDir)
			err = nil
		default:
			return err
		}
...
func (g *GitRepo) CommitAndPush(message string) error {
	var err error
	repo, err := git.PlainOpenWithOptions(g.WorkingDir, &git.PlainOpenOptions{DetectDotGit: true, EnableDotGitCommonDir: true})
	if err != nil {
		return err
	}
	w, err := repo.Worktree()
